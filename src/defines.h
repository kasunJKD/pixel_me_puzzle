...
Allocator make_bump_allocator(size_t size)
{
  Allocator ba = {};
...
char* bump_alloc(Allocator* allocator, size_t size)
{
  char* result = nullptr;
  ...
  {
    SM_ASSERT(false, "Allocator is full");
  }
...
char* read_file(char* filePath, int* fileSize, Allocator* allocator)
{
  char* file = nullptr;
  ...
  if(fileSize2)
  {
    char* buffer = bump_alloc(allocator, fileSize2 + 1);
...
bool copy_file(char* fileName, char* outputName, Allocator* allocator)
{
  char* file = 0;
